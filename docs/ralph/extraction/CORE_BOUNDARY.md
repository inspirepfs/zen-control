...
| Codex executable, app-server usage/model catalog, and configured default model/reasoning effort | controller subprocess/configuration helpers | `adapter-required` | A runtime adapter supplies the executable plus usage, model-catalog, supported-reasoning-effort, and configured-default capability. |

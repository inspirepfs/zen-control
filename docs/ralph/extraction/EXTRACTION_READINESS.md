...
| Codex executable/app-server and `$HOME/.codex/config.toml` model/reasoning-effort lookup | Runtime-state dependency | `run_codex`, model-catalog/rate-limit helpers, configured/selected model and reasoning-effort helpers | Define a model-runner and usage-provider interface that carries model plus reasoning effort. Current local executable and user-home configuration are required. |

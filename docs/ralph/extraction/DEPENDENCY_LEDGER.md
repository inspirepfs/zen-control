...
| DL-08 | The `codex` executable and its app-server stdio interface provide plan/step execution, model catalog (including supported reasoning efforts), rate limits, and reset-credit operations; the global Codex config may provide default model and `model_reasoning_effort` values. | `scripts/ralph.py` Codex helpers; `~/.codex/config.toml` lookup | `EXTERNAL_TOOL` | critical | Define the executable/app-server contract separately from the project-local model/reasoning-effort overrides. | Missing/auth-incompatible Codex blocks proposal/run and usage/model features. |
...
| DL-10 | Policy schemas are `zen_ralph_efficiency_policy_v2` and `zen_ralph_model_policy_v2`; efficiency and model policy both retain their documented legacy normalization paths (model v1 migrates with no effort override). | `scripts/ralph_efficiency.py`, `scripts/ralph_model.py` | `COMPATIBILITY` | conditional | Preserve schemas, migration behavior, and validation limits until a separately approved migration. | Policy loss or schema drift changes resource/model/reasoning-effort selection. |

...
### Upgrade safety and schema lineage

From v0.57 the SQLite policy/configuration store has an explicit schema version (`PRAGMA user_version=570`) plus a `schema_migrations` audit row. Existing unversioned v0.56-and-earlier databases are treated as historical inputs: ZEN validates them and creates a verified recovery copy before running any compatibility CREATE/ALTER/backfill work, then advances the formal schema marker only after the whole migration path succeeds. A database from a newer schema version fails closed.

The host release helper adds a second, external recovery boundary. Before rebuilding `mikrotik-control`, it uses SQLite's online backup API against the live Docker data volume, writes the coherent copy outside the Compose volume, validates it, and runs `scripts/upgrade_acceptance.py` against a temporary restore copy. Runtime health then proves the live database reached the current schema after restart. None of these paths imports RouterOS code or obtains policy mutation authority.
### Reporting and analytics

v0.58 adds a dedicated reporting composition layer over evidence ZEN already retains. `/reporting` and `/api/reporting/overview` compare the selected activity window with the immediately preceding equivalent window, expose daily traffic/DNS/classification trends, rank device and service movers, and combine notification/incident lifecycle statistics with durable desired-policy checkpoint signals. The 7-day default is published as a revision-bound prepared view so the normal report does not add a large synchronous PostgreSQL fan-out to navigation. Custom windows remain bounded by the existing retained-activity range contract.

Reporting deliberately does not create a new truth source. Network activity remains IPFIX/Pi-hole evidence; desired-policy and quota/schedule statistics are labelled as **checkpoints**, not duration or historical RouterOS execution; notification and incident figures are durable lifecycle evidence; missing classification evidence remains UNKNOWN/UNAVAILABLE rather than becoming zero. The reporting module imports no RouterOS adapter and owns no write or mutation path. CSV export is a presentation/export surface over the same read-only report contract.

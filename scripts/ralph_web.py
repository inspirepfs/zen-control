...
def snapshot(usage_report: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    model_selection = model_policy.load_policy(ROOT)
    plan = state.get("plan") if isinstance(state.get("plan"), dict) else {}
...
<section class="card span12"><div class="section-title-row"><h2>Efficiency / Resource Controls</h2><span id="effPolicyState" class="badge">live</span></div><div id="efficiencyControls" class="efficiency-controls"></div></section>
...
const renderedValues=new WeakMap(),pendingTargetUpdates=new Map();let renderedControlsIdentity=null,latestSnapshot=null,refreshGeneration=0,actionFeedback=null,localActionState=null,efficiencyDirty=false;
...
function renderEfficiency(s,force=false){if(!s)return;if(efficiencyDirty&&!force)return;const p=s.efficiency_policy||{},d=s.efficiency_defaults||{};const recommendation=String(s.controller?.efficiency_recommendation||'NORMAL');const field=(key,label,help,step='1',min='0',modeLimit=false)=>{const value=p[key]??d[key]??'';const changed=Number(value)!==Number(d[key]);return `<div class="eff-field" ${modeLimit?'data-mode-limit':''}><div><label>${info(label,help)}</label><input data-eff-key="${esc(key)}" type="number" step="${esc(step)}" min="${esc(min)}" value="${esc(value)}" oninput="markEfficiencyDirty()"><div class="baseline">baseline ${esc(d[key]??'-')}</div></div><button type="button" class="icon-btn inline-reset" data-eff-reset="${esc(key)}" title="Reset ${esc(label)} to baseline" style="visibility:${changed?'visible':'hidden'}" onclick="resetEfficiencyField('${esc(key)}')">↺</button></div>`;};const mode=String(p.mode||'NORMAL').toUpperCase(),modeChanged=mode!==String(d.mode||'NORMAL').toUpperCase();const modeField=`<div class="eff-field"><div><label>${info('Efficiency mode','STRICT applies tighter per-turn limits; NORMAL is the baseline; RELAXED allows broader high-context work; OFF disables ordinary efficiency pauses but not admission reserve or emergency runaway protection.')}</label><select data-eff-key="mode" onchange="markEfficiencyDirty()"><option value="STRICT" ${mode==='STRICT'?'selected':''}>Strict</option><option value="NORMAL" ${mode==='NORMAL'?'selected':''}>Normal</option><option value="RELAXED" ${mode==='RELAXED'?'selected':''}>Relaxed</option><option value="OFF" ${mode==='OFF'?'selected':''}>Off</option></select></div><button type="button" class="icon-btn inline-reset" data-eff-reset="mode" title="Reset mode to NORMAL" style="visibility:${modeChanged?'visible':'hidden'}" onclick="resetEfficiencyField('mode')">↺</button></div>`;const modePanel=(name)=>{const k=name.toLowerCase();return `<div class="eff-mode-panel" data-mode-panel="${name}" ${mode===name?'':'hidden'}><div class="small"><b>${name}</b> limits — editable only while this mode is selected.</div><div class="eff-mode-limits">${field(`${k}_prompt_command_budget`,'Prompt commands','Maximum shell-command executions requested inside one model turn.','1','1',true)}${field(`${k}_max_commands`,'Observed commands','Pause after a qualified step if the model reports more command executions than this.','1','1',true)}${field(`${k}_max_reported_files`,'Files inspected','Pause after a qualified step if reported file inspection exceeds this limit.','1','1',true)}${field(`${k}_max_cumulative_input`,'Input tokens','Maximum cumulative input tokens for one turn before the selected-mode efficiency pause.','1000','1000',true)}${field(`${k}_max_noncached_input`,'Non-cached input','Maximum non-cached input tokens for one turn before the selected-mode efficiency pause.','1000','1000',true)}</div></div>`;};const html=`<div class="eff-note">Live policy: staged edits take effect together when <b>Apply changes</b> is pressed. RALPH reloads policy between model turns. The admission reserve never revokes an already-admitted plan, and the emergency ceiling stays active in OFF mode.</div><div class="eff-toolbar">${modeField}${field('reserve_percent','New-work reserve %','Minimum remaining Codex allowance required to admit a new plan. An admitted plan may continue below this reserve.','0.5','0')}<div class="small">Planner recommendation: <b>${esc(recommendation)}</b><br>Current policy revision ${esc(p.revision||'-')}</div></div><div id="effOffNote" class="disabled-note" ${mode==='OFF'?'':'hidden'}><b>OFF:</b> ordinary efficiency thresholds are disabled and therefore not editable. Admission reserve and emergency runaway protection remain active.</div>${modePanel('STRICT')}${modePanel('NORMAL')}${modePanel('RELAXED')}<details class="eff-advanced"><summary>Emergency runaway ceiling</summary><div class="small">Always enforced, including OFF. Keep these above every enabled mode limit.</div><div class="eff-mode-limits">${field('runaway_max_commands','Runaway commands','Absolute emergency command ceiling, independent of efficiency mode.')}${field('runaway_max_reported_files','Runaway files','Absolute emergency file-inspection ceiling, independent of efficiency mode.')}${field('runaway_max_cumulative_input','Runaway input','Absolute emergency cumulative-input ceiling.','1000','1000')}${field('runaway_max_noncached_input','Runaway non-cached','Absolute emergency non-cached-input ceiling.','1000','1000')}</div></details><div class="eff-actions"><button class="good" type="button" onclick="submitEfficiencySettings()">Apply changes</button><button type="button" onclick="stageEfficiencyDefaults()">Restore baselines</button><button type="button" onclick="discardEfficiencyChanges()">Discard staged</button><span class="small">updated ${esc(p.updated_at||'defaults')}</span></div>`;efficiencyDirty=false;renderHTML('efficiencyControls',html);renderText('effPolicyState','live');renderClass('effPolicyState','badge ok');syncEfficiencyResetIcons();syncEfficiencyModePanels();}
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if result.get("ok") and str(payload.get("action") or "") in {
                "redeem_reset", "usage_reset_stats", "model_update", "model_reset",
            }:
                monitor = getattr(self.server, "usage_monitor", None)

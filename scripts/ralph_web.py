...
def command_for_action(payload: dict[str, Any], state: dict[str, Any]) -> CommandRequest:
    ...
    if action == "authorize_self_hosting":
        ...
        argv = ["authorize-self-hosting", str(state.get("plan_hash") or ""), "--gate", expected_gate]
...
function renderActionFailure(action,message){renderHTML('actionResult',`<div class="action-title bad">${esc(actionLabel(action))} failed</div><div>${esc(message||'action failed')}</div>`);}
...
async function submitSelfHosting(){const reason=document.getElementById('selfHostingReason')?.value.trim()||'';const gate=document.getElementById('gateId')?.textContent.trim()||'';const button=document.getElementById('selfHostingButton');const paths=[...document.querySelectorAll('input[data-self-host-path]:checked')].map(x=>x.value);if(!reason){renderActionFailure('authorize_self_hosting','Operator reason is required.');return;}if(!paths.length){renderActionFailure('authorize_self_hosting','At least one controller candidate path is required.');return;}if(button)button.disabled=true;try{const j=await post({action:'authorize_self_hosting',gate,paths,reason});renderActionResult('authorize_self_hosting',j);await refresh();}catch(e){renderActionFailure('authorize_self_hosting',e.message);}finally{if(button)button.disabled=false;}}

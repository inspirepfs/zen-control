...
def command_for_action(payload: dict[str, Any], state: dict[str, Any]) -> CommandRequest:
    ...
    if action == "propose_replacement":
        ...
        goal = str(payload.get("goal") or "").strip()
        argv = ["propose", "--from-retirement", retirement["record_id"]]
        if "min_steps" in payload or "max_steps" in payload:
            minimum, maximum = _proposal_bounds(payload)
            argv += ["--min-steps", str(minimum), "--max-steps", str(maximum)]
        if goal:
...
async function submitReplacement(recordId){if(String(latestSnapshot?.latest_retirement?.record_id||'')!==String(recordId||'')){renderActionFailure('propose_replacement','The current controller retirement record is no longer available.');return;}const minSteps=Number(document.getElementById('planMinSteps')?.value||5),maxSteps=Number(document.getElementById('planMaxSteps')?.value||10),goal=String(document.getElementById('replacementGoal')?.value||'').trim();try{showActionState('propose_replacement');const j=await post({action:'propose_replacement',retirement_record_id:recordId,min_steps:minSteps,max_steps:maxSteps,goal});renderActionResult('propose_replacement',j);await refresh();}catch(e){renderActionFailure('propose_replacement',e.message);}finally{localActionState=null;}}
...
async function submitGoal(){const goal=document.getElementById('goalInput').value.trim();if(!goal)return;const minSteps=Number(document.getElementById('planMinSteps')?.value||5),maxSteps=Number(document.getElementById('planMaxSteps')?.value||10);try{showActionState('propose');await post({action:'propose',goal,min_steps:minSteps,max_steps:maxSteps});await refresh();}catch(e){renderActionFailure('propose',e.message);}finally{localActionState=null;}}

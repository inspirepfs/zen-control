...
def snapshot(usage_report: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "controller": {
            ...
            "efficiency_mode": str(state.get("efficiency_mode") or "NORMAL"),
...
def command_for_action(payload: dict[str, Any], state: dict[str, Any]) -> CommandRequest:
    ...
    if action == "run":
        ...
        mode = str(payload.get("efficiency_mode") or state.get("efficiency_mode") or "NORMAL").strip().lower()
        if mode not in {"strict", "normal", "relaxed", "off"}:
            raise WebConsoleError("efficiency_mode must be strict, normal, relaxed, or off")
        return CommandRequest(["run", "--color", "never", "--max-loops", str(max_loops), "--efficiency-mode", mode], background=True, activity="RUNNING")
    if action == "steer":
...
@media(max-width:1150px){.span3{grid-column:span 6}.span4,.span5,.span6,.span7,.span8,.span9{grid-column:span 12}.usage-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.usage-row{grid-template-columns:100px 80px minmax(210px,240px) minmax(0,1fr)}}
...
async function submitRun(){const mode=document.getElementById('efficiencyMode')?.value||'normal';try{showActionState('run');const j=await post({action:'run',max_loops:40,efficiency_mode:mode});renderActionResult('run',j);await refresh();}catch(e){renderActionFailure('run',e.message);}finally{localActionState=null;}}

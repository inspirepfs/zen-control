...
function renderUsage(u,snapshot){const p=u.current_plan||{};const ratio=p.input_tokens?((Number(p.cached_input_tokens||0)/Number(p.input_tokens))*100):0;const wins=u.windows||[];const metrics=[`<div class="usage-box"><div class="small">Plan input</div><div class="usage-value">${num(p.input_tokens)}</div><div class="small">non-cached ${num(p.noncached_input_tokens)}</div></div>`,`<div class="usage-box"><div class="small">Plan output</div><div class="usage-value">${num(p.output_tokens)}</div><div class="small">reasoning ${num(p.reasoning_output_tokens)}</div></div>`,`<div class="usage-box"><div class="small">Cached input</div><div class="usage-value">${num(p.cached_input_tokens)}</div><div class="small">cache ${ratio.toFixed(1)}%</div></div>`,`<div class="usage-box"><div class="small">Observed turns</div><div class="usage-value">${num(p.turns)}</div><div class="small">${esc(u.model||'-')} · ${esc(u.plan_type||'-')}</div></div>`];for(const w of wins.slice(0,2)){const t=w.observed_tokens||{};metrics.push(`<div class="usage-box usage-window"><div class="small">${esc(w.name||w.slot||'window')}</div><div class="usage-value">${Number(w.remaining_percent??0).toFixed(1)}% left</div><div class="small">reset ${esc(when(w.resets_at))}</div><div class="small">in ${num(t.input_tokens)} · out ${num(t.output_tokens)}</div></div>`);}renderHTML('usageSummary',metrics.join(''));renderHTML('usageWindows','');const catalog=u.model_catalog||{},models=catalog.models||[],override=String(snapshot?.model_policy?.model||'');const modelOptions=[`<option value="" ${override?'':'selected'}>Codex default${catalog.configured_default?' · '+esc(catalog.configured_default):''}</option>`,...models.map(m=>`<option value="${esc(m.id)}" ${override===String(m.id)?'selected':''}>${esc(m.display_name||m.id)}</option>`)].join('');renderHTML('topModelControls',`<select aria-label="RALPH model" title="RALPH model for the next model turn. Changing this does not interrupt an in-flight Codex turn." onchange="selectModel(this.value)">${modelOptions}</select>${override?`<button class="icon-btn inline-reset" title="Return to Codex configured default model" onclick="selectModel('')">↺</button>`:''}<button class="danger-solid" title="Reset only RALPH local token statistics" onclick="resetTokenStats()">Reset token stats</button>`);const resetCount=Number(u.available_reset_credits??0);const credits=(u.reset_credits||[]).filter(c=>String(c.status||'').toLowerCase()==='available');const chosen=credits[0]||{};const redeem=resetCount>0?`<button class="sparkle" title="${resetCount} banked reset${resetCount===1?'':'s'} available${chosen.expires_at?' · earliest expiry '+when(chosen.expires_at):''}" onclick="redeemBankedReset()">Redeem${resetCount>1?' · '+resetCount:''}</button>`:'';renderHTML('usageToolbar',redeem);const plans=(u.plans||[]).slice(0,10);const planRows=plans.map(x=>{const goal=String(x.goal||'No plan comment recorded.'),hash=(x.plan_hash||'unassigned').slice(0,8),cache=Number(x.cache_ratio_percent||0).toFixed(1),planModels=(x.models||[]).join(', ')||'-';return `<details class="plan-usage-detail"><summary class="plan-usage-summary"><span class="plan-hash">${x.current?'<span class="current-tag">●</span> ':''}${esc(hash)}</span><span class="plan-number">${num(x.turns)} turns</span><span class="plan-number hide-mobile">${num(x.total_tokens)}</span><span class="plan-number hide-mobile">${cache}%</span><span class="plan-goal" title="${esc(goal)}">${esc(goal)}</span></summary><div class="plan-usage-meta"><div class="plan-mini"><div class="small">Input / non-cached</div><b>${num(x.input_tokens)}</b> / ${num(x.noncached_input_tokens)}</div><div class="plan-mini"><div class="small">Cached / cache-write</div><b>${num(x.cached_input_tokens)}</b> / ${num(x.cache_write_input_tokens)}</div><div class="plan-mini"><div class="small">Output / reasoning</div><b>${num(x.output_tokens)}</b> / ${num(x.reasoning_output_tokens)}</div><div class="plan-mini"><div class="small">Average / turn</div><b>${num(x.avg_input_tokens)} in</b> · ${num(x.avg_output_tokens)} out</div><div class="plan-mini"><div class="small">Observed</div>${esc(when(x.first_epoch))} → ${esc(when(x.last_epoch))}</div><div class="plan-mini"><div class="small">Models</div>${esc(planModels)}</div><div class="plan-breakdown">${breakdownTable('Scope',x.scopes)}${breakdownTable('Phase',x.phases)}${breakdownTable('Step',x.steps)}</div></div></details>`;}).join('');const header=plans.length?'<div class="plan-usage-head"><span>Plan</span><span>Turns</span><span>Total tokens</span><span>Cache</span><span>Goal</span></div>':'';const resetNote=u.stats_reset?`<div class="small">Local stats baseline reset ${esc(u.stats_reset.reset_at||'')}</div>`:'';renderHTML('planUsage',header+(planRows||'<div class="small">No usage-ledger history since the current statistics baseline.</div>')+resetNote);}

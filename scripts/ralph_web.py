...
function renderActionResult(action,j){const body=[prettyOutput(j.stdout),prettyOutput(j.stderr)].filter(Boolean).join('');document.getElementById('actionResult').innerHTML=`<div class="action-title">${esc(actionLabel(action))}</div>${body||'<div class="small">Controller accepted the action. Current state is shown above.</div>'}`;}
...
function renderUsage(u){const p=u.current_plan||{};const ratio=p.input_tokens?((Number(p.cached_input_tokens||0)/Number(p.input_tokens))*100):0;document.getElementById('usageSummary').innerHTML=`<div class="usage-box"><div class="small">Plan input</div><div class="usage-value">${num(p.input_tokens)}</div><div class="small">non-cached ${num(p.noncached_input_tokens)}</div></div><div class="usage-box"><div class="small">Plan output</div><div class="usage-value">${num(p.output_tokens)}</div><div class="small">reasoning ${num(p.reasoning_output_tokens)}</div></div><div class="usage-box"><div class="small">Cached input</div><div class="usage-value">${num(p.cached_input_tokens)}</div><div class="small">cache ${ratio.toFixed(1)}%</div></div><div class="usage-box"><div class="small">Observed turns</div><div class="usage-value">${num(p.turns)}</div><div class="small">${esc(u.model||'-')} · ${esc(u.plan_type||'-')}</div></div>`;const wins=u.windows||[];document.getElementById('usageWindows').innerHTML=wins.map(w=>{const t=w.observed_tokens||{};return `<div class="usage-row"><span>${esc(w.name||w.slot||'window')}</span><span>${Number(w.remaining_percent??0).toFixed(1)}% left</span><span>in ${num(t.input_tokens)} · out ${num(t.output_tokens)}</span><span>reset ${esc(when(w.resets_at))}</span></div>`;}).join('')||'<div class="small">Live quota windows are refreshing…</div>';document.getElementById('planUsage').innerHTML=(u.plans||[]).slice(0,8).map(x=>`<div class="usage-row"><span>${esc((x.plan_hash||'unassigned').slice(0,8))}</span><span>${num(x.turns)} turns</span><span>in ${num(x.input_tokens)} · out ${num(x.output_tokens)}</span><span>${esc((x.goal||'').slice(0,80))}</span></div>`).join('')||'<div class="small">No v0.3.2 usage-ledger history yet.</div>';}
function render(s){document.getElementById('version').textContent='v'+s.version;const c=s.controller,p=s.plan,g=s.git,j=s.job,u=s.usage||{};document.getElementById('status').textContent=c.status;document.getElementById('controller').innerHTML=kv({plan:(c.plan_hash||'-').slice(0,16),step:`${c.current_step}/${c.step_count||'-'}`,loops:c.loop_count,recovery:c.recovery_checkpoint||'-'});document.getElementById('planMetric').textContent=`${Math.min(c.current_step,c.step_count||0)}/${c.step_count||'-'}`;document.getElementById('planMeta').innerHTML=kv({steering:c.steering_count,commit:(c.commit_sha||'-').slice(0,12),upstream:c.push_upstream||'-'});document.getElementById('quota').textContent=c.quota_remaining_percent==null?'-':c.quota_remaining_percent.toFixed(1)+'%';document.getElementById('eff').innerHTML=kv({efficiency:c.efficiency,job:j.active?`RUNNING pid ${j.pid}`:'idle',limits:u.captured_at||'refreshing'});document.getElementById('branch').textContent=g.branch;document.getElementById('git').innerHTML=kv({upstream:g.upstream||'-',dirty:g.dirty?`${g.dirty_count} files`:'clean',head:(g.head||'-').slice(0,12)});renderUsage(u);document.getElementById('goal').textContent=p.goal||'No active plan.';document.getElementById('steps').innerHTML=p.steps.map(x=>`<div class="step ${esc(x.state)}"><div class="step-title">${x.id}. ${esc(x.title)} <span class="badge">${esc(x.state)}</span></div><div>${esc(x.objective)}</div><div class="small">Test changes: ${esc(x.test_change_policy)}</div>${(x.acceptance||[]).length?`<div class="small"><b>Acceptance criteria</b></div><ul class="criteria">${x.acceptance.map(a=>`<li>${esc(a)}</li>`).join('')}</ul>`:''}</div>`).join('')||'<div class="small">No plan steps.</div>';document.getElementById('gate').innerHTML=s.gate?`<div class="notice gate"><div><b id="gateId">${esc(s.gate.id)}</b> · Step ${s.gate.step}</div><div class="step-title">${esc(s.gate.title||'Human review')}</div><div class="small">${s.gate.policy_review?'POLICY / AUTHORITY REVIEW':'OPERATOR EVIDENCE REVIEW'} · tests=${esc(s.gate.test_change_policy)}</div><h3>Why Ralph stopped</h3>${prettyOutput(s.gate.block_reason)}${(s.gate.acceptance||[]).length?`<h3>Acceptance criteria</h3><ul class="criteria">${s.gate.acceptance.map(a=>`<li>${esc(a)}</li>`).join('')}</ul>`:''}<h3>Recommended action</h3><div>${esc(s.gate.recommendation||'Review the evidence and choose a bounded operator action.')}</div></div>`:'<div class="small">No open human gate.</div>';renderControls(s);const ev=document.getElementById('events');ev.innerHTML=s.events.map(e=>`<div class="event ${esc(e.category||'')}"><b>${esc(e.category||'EVENT')}</b> ${esc(e.message||'')}</div>`).join('');ev.scrollTop=ev.scrollHeight;document.getElementById('files').innerHTML=(c.plan_changed_files||[]).map(x=>`<div class="file">${esc(x)}</div>`).join('')||'<div class="small">No recorded plan files yet.</div>';document.getElementById('report').textContent=s.report?.preview||'No completion report yet.';document.getElementById('log').textContent=(s.live_log||[]).join('\n')||'No output yet.';document.getElementById('job').textContent=j.active?`job RUNNING ${j.pid}`:'job idle';document.getElementById('job').className='badge '+(j.active?'info':'');document.getElementById('refresh').textContent='live';document.getElementById('refresh').className='badge ok';}
async function refresh(){try{const r=await fetch('/api/snapshot',{cache:'no-store'});if(r.status===401){location.reload();return;}if(!r.ok)throw new Error('snapshot '+r.status);const s=await r.json();render(s);document.getElementById('error').innerHTML='';}catch(e){document.getElementById('refresh').textContent='offline';document.getElementById('refresh').className='badge bad';showError(e.message);}}

...
- Static critical firewall authority remains explicitly and manually owned on RouterOS.
- Built-in service contracts remain read-only; ZEN Control validates them but never creates, reorders or silently repairs their `RW*` rules.
- Custom services are reporting-only by default. A custom TLS/SNI contract is created only after an operator previews and explicitly approves its deterministic `MC|SVC|*` rules and `MC_*` address-list namespace.
- App-owned custom contracts are fresh-read after every install/remove operation, fail closed on conflicts or partial state, and never overwrite unrelated/manual rules.
- MikroTik Kid Control remains untouched as an independent safety net.
- Automatic enforcement is held closed when critical authority validation fails.
- Telemetry loss does not disable RouterOS policy enforcement and quota evaluation fails open rather than inventing usage.

## Main capabilities

- NORMAL / SLOW / BLOCKED global and per-device modes
- Dynamic service blocking backed by validated RouterOS classifiers
- Explicit custom-service RouterOS provisioning with preview, approval, idempotent install, conflict detection, post-write validation and safe removal
- Profiles, policy groups, templates and device assignments
- Bandwidth presets and per-device queues
- Recurring schedules and date exceptions
- Automatic reconciliation with cooldown/circuit-breaker behaviour
- Temporary parent-granted access and reward-time banks
- Daily total/service/group data quotas
- Security authority validation and bypass evidence
- Durable incidents, audit, configuration checkpoints and sanitized operational diagnostics
- TOTP parent authentication with multiple authenticators and shared-display locking
- IPFIX + Pi-hole + PostgreSQL activity reporting
- Service intelligence with validated RouterOS TLS/SNI coverage, dynamic DNS classifiers and per-service drill-down
- Historical parent analytics with period comparisons, daily classification trends, new-domain discovery, active periods and a chronological network-evidence timeline
- Daily Parent Summary Center with managed-device comparisons, top services, blocked/new DNS attention and current quota signals
- Opt-in Parent Summary Delivery with daily policy-timezone scheduling, durable idempotent outbox/history, bounded retries, email and webhook/Home Assistant transports
- Connected system-health overview across policy, RouterOS authority, service intelligence, telemetry and operations
- Context-aware cross-navigation between managed devices, policy assignments, activity, incidents and audit evidence
- Effective-policy explainability with a read-only decision chain for device mode, schedules/exceptions, quotas, temporary access, global mode, service provenance, bandwidth and fresh RouterOS state
- Device 360 operational view joining policy, access/reward/quota state, service decisions, today's activity, DNS attention, recent incidents and audit evidence for one managed device
- Responsive dashboard for desktop, tablet and mobile
- Context-aware in-product help with subsection/tool-specific guidance, evidence boundaries, glossary and searchable Help Center

## Deployment

The supported deployment is the single `docker-compose.yml` in the project root. It starts the control application, GoFlow2, PostgreSQL telemetry, Pi-hole and the telemetry ingest worker.
...
The dashboard is exposed on port `8080` by default. Pi-hole is exposed on the configured LAN address in `docker-compose.yml`.

## Parent authentication

Password login remains available until the account is deliberately changed to OTP-only mode. Each parent authenticator has its own TOTP secret and can be revoked independently. Shared-display mode keeps the dashboard visible while write controls remain server-side locked until a parent enters a current OTP or recovery code.

Keep `SESSION_SECRET` and `OTP_ENCRYPTION_KEY` stable and backed up. Losing the OTP encryption key invalidates encrypted authenticator secrets; recovery codes are the break-glass path.

## Telemetry

RouterOS exports IPFIX to GoFlow2. The ingest worker correlates flows with Pi-hole DNS data and stores retained activity in PostgreSQL. The application publishes the live Policy → Services classifier catalogue into the shared data volume, so custom DNS/TLS service signatures are picked up by telemetry without rebuilding the ingest image. Custom definitions therefore participate in reporting immediately, even when they are intentionally reporting-only. Built-in service TLS/SNI learner contracts are validated read-only against RouterOS and surfaced with detector counts. Explicitly approved custom TLS/SNI services additionally expose a deterministic RouterOS contract and become eligible for live policy enforcement only while that contract is locally approved and fresh RouterOS validation succeeds. Removing a custom contract or deleting its current definition does not purge retained PostgreSQL activity history. Activity reporting remains evidence-led: service/domain attribution is shown only when telemetry supports it, and generic HTTPS traffic is not guessed into a service.


## Custom service enforcement

Custom service metadata and RouterOS authority are deliberately separate. Saving DNS/TLS classifier metadata never writes firewall rules. For a provisionable custom service, Policy → Services shows the exact deterministic contract before any change is made. The operator must explicitly approve installation; ZEN Control then verifies the core enforcement gate, checks for manual/namespace conflicts, creates only its `MC|SVC|<key>|*` rules before `RW99 - Return`, reconnects to RouterOS and validates the result before recording local approval.

Removal is equally conservative: ZEN Control removes only a complete, validated app-owned contract and its deterministic custom address-list entries. Partial or conflicting state is refused for manual investigation instead of being guessed or repaired. Logical groups such as Gaming and Social media remain policy expansion constructs and never receive aggregate `MC_Block_<Group>` firewall rules.

## Parent summary

Activity → Summaries provides a read-only daily parent digest over managed devices. Today and yesterday views compare each device with the immediately preceding equivalent local-time window and surface traffic, DNS counts, blocked DNS evidence, new domains relative to a 30-day lookback, service attribution and drill-down links. Today additionally shows current quota warning/exhaustion state where quotas are configured. Historical quota state is deliberately not reconstructed from today's policy configuration.

The summary does not assign an opaque risk score and does not infer browser history, foreground application use, the identity of the person using a device, or intent. DNS attention tags are deterministic evidence labels: `NEW`, `BLOCKED` and `UNCLASSIFIED`.

## Parent summary delivery

Settings → Automation → Parent summary delivery consumes the exact Parent Summary contract above rather than issuing a second set of telemetry queries. Scheduled delivery is disabled by default and uses the configured policy timezone. The operator chooses `today` or `yesterday`, an HH:MM delivery time, and one or both explicit transports.

Email recipients and webhook URL are stored as normal configuration in `policy.db`. SMTP host, credentials, sender address and the optional webhook bearer token remain environment-only secrets. Each scheduled report/channel gets one durable SQLite outbox identity. Retries reuse that identity; webhooks receive an `Idempotency-Key` header and email retries reuse a stable `Message-ID`. This gives strong application-side de-duplication while remaining technically honest about the unavoidable at-least-once crash boundary after an external server accepts a message but before local success is committed. Delivery history, attempts, failures and 30-day success percentage are visible in the UI. Test sends and terminal-failure retries require an unlocked admin session and are audited.

No delivery operation changes RouterOS, Kid Control, policy state or historical telemetry.

## Connected UX and feature integration

Dashboard → Connected system health composes existing evidence from the control plane, RouterOS security authority, service contracts, telemetry, incident state and operations into five dense status rows. It is a presentation layer only: it does not create new policy, infer behaviour or write RouterOS. Each row links directly to the feature that owns the underlying state.

Cross-feature drill-downs retain context. Favourites and managed-device cards link into policy assignment and activity; activity device/service pages link back into the corresponding control-plane configuration; Policy Summary links to the exact managed-device and assignment views; incidents link to their owning evidence area; and recent audit events are searchable and link back to the relevant operational surface. Deep links can focus and briefly highlight the exact device, assignment or service row.

POST actions now return to the exact subsection that owns the change rather than broad legacy anchors. This keeps the compact subnavigation stable after edits and makes policy, schedule, service, security, incident and operations workflows feel continuous.

## Effective policy explainability

ZEN Control can answer **why a managed device or concrete service is in its current state** without introducing another policy engine. `/policy/explain/<ip>` composes the existing effective desired policy with fresh RouterOS evidence and shows the ordered decision chain: assigned profile/default, per-device override, active schedule or date exception, quota evaluation, temporary RouterOS access, computed desired mode, live per-device mode and household global MASTER mode. It also shows the next known policy-clock or temporary-access transition.

The same view explains each concrete service as `ALLOW`, `BLOCK` or `BLOCK REQUESTED`, with provenance from direct profile membership, logical policy-group expansion, service/group schedules or quota exhaustion. Reporting-only custom services are never presented as enforced: without a trusted RouterOS contract their live state is `NO CONTRACT`. RouterOS/service drift is shown explicitly, and the JSON form is available at `/api/policy/explain/<ip>`. The explainer is read-only and never creates, repairs or reorders RouterOS authority.

Managed-device cards, favourites, Policy Summary, device activity and service activity link into this explanation surface, including focused service explanations such as `/policy/explain/<ip>?service=youtube`.

## Device 360

`/devices/<ip>` is the compact operational landing page for one managed device. It consumes the existing `zen_policy_explanation_v1` contract rather than re-running or re-implementing policy decisions, and adds current reward/quota/access state, today's retained activity comparison, service attribution, explicit blocked/new DNS evidence, recent network evidence, and incident/audit rows that explicitly mention the device IP or display name. The JSON form is available at `/api/devices/<ip>/360` as `zen_device_360_v1`.

Device 360 is read-only. Changes continue through the existing managed-device, policy, schedule and parent-unlock/write-gated surfaces. Telemetry failure degrades only the activity portion: missing PostgreSQL/Pi-hole evidence is shown as unavailable and is never converted to zero activity. Incident and audit correlation is text-evidence based rather than fuzzy behavioural inference, and Device 360 does not claim browser history, foreground application time, user identity or a risk score.

## Progressive Web App / Android tablet
...
- `/health/ready` — database, RouterOS, security and worker readiness
- `/health` — compatibility alias for readiness
- `/api/operations/status` — operations state
- `/api/operations/diagnostics` — sanitized read-only cross-component diagnostic contract (`zen_operational_diagnostics_v1`)
- `/diagnostics` — authenticated operational diagnostics workbench and support-bundle export
- `/api/policy/explain/<ip>` — read-only effective-policy and live RouterOS explanation contract
- `/api/policy/quality` — read-only static conflict, shadow, redundancy and unused-policy analysis
- `/api/devices/<ip>/360` — read-only composed managed-device operational contract
- `/api/security/posture` — RouterOS enforcement/security posture
- `/api/services/health` — built-in and custom RouterOS service-contract health, including reporting-only, healthy, degraded and orphaned custom states
- `/api/activity/services` — dynamic service activity, classification coverage and unknown-domain candidates
- `/api/activity/summary` — read-only today/yesterday managed-device parent summary contract
- `/api/activity/policy-history` — read-only historical desired-policy checkpoint × retained telemetry correlation (`zen_policy_correlation_v1`)
- `/api/summary-delivery` — delivery configuration readiness, durable outbox statistics and recent history
- `/api/incidents` — durable incident state
- `/api/pwa/status` — authenticated PWA/install/security contract (`zen_pwa_status_v1`)
- `/help` — authenticated searchable Context & Help Center
- `/api/help` — static read-only help/glossary contract (`zen_help_catalog_v1`), with no household data
- `/api/performance` — bounded in-memory v0.30 route/component/query performance evidence
- `/performance` — authenticated performance evidence UI and same-origin browser baseline probe

Configuration snapshots and durable audit data are stored in the application data volume alongside `policy.db`.

## Testing

Use an isolated virtual environment so optional application dependencies such as QR-code generation are available:

```bash
python3 -m py_compile app/*.py && echo "Python: PASS"
python3 scripts/ux_validate.py
python3 -m unittest discover -s tests -v
docker compose -f docker-compose.yml up -d --build mikrotik-control
docker compose logs --tail=100 mikrotik-control
```

For a clean host or CI environment, install `requirements.txt` first. The two Docker commands above mirror the supported application-only rebuild/log check used during iterative upgrades.

## Repository housekeeping

Historical one-off migration compose fragments, embedded development patches, the old ClickHouse prototype and standalone router smoke script have been removed. The root `docker-compose.yml`, PostgreSQL telemetry schema and current application modules are the supported sources of truth.
